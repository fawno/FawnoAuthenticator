<?php
	...
	namespace FawnoAuthenticator\Authenticator;
	...
	class IPAuthenticator extends AbstractAuthenticator implements PersistenceInterface {
		...
		public function authenticate (ServerRequestInterface $request) : ResultInterface {
			...
			if ($request instanceof ServerRequest) {
				...
				$clientIp = current(explode(',', $server['HTTP_FASTLY_CLIENT_IP'] ?? $server['HTTP_X_FORWARDED_FOR'] ?? $server['REMOTE_ADDR']));
			}

			$user = $auth_ips[$clientIp] ?? null;
